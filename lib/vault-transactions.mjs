...
export function createTransactionEngine({
  ...
  async function prepareTransaction({
    ...
    if (!all && typeof amount !== "string")
      throw fail("Enter the amount as a decimal USDC value.", "INVALID_AMOUNT");
    ...
    if (kind === "deposit") {
      ...
      if (rawAmount > position.cash)
        throw fail(
          "This amount exceeds your USDC balance on the selected network.",
          "INSUFFICIENT_CASH",
        );
  ...
  async function executeTransaction({
    ...
    if (action === "approve" && fresh.kind !== "deposit")
      throw fail(
        "This withdrawal does not need a USDC approval.",
        "INVALID_APPROVAL",
      );
    if (action === "approve" && !fresh.needsApproval)
      throw fail(
        "USDC is already approved for this amount. Continue to deposit.",
        "ALREADY_APPROVED",
      );
    if (action === "submit" && fresh.needsApproval)
      throw fail(
        "Approve this USDC amount before depositing.",
        "APPROVAL_REQUIRED",
      );
    ...
    if (action === "approve" && simulation.result === false)
      throw fail(
        "USDC did not accept the approval in simulation. No approval was sent.",
        "APPROVAL_REJECTED",
      );
    ...
    if (action === "approve") {
      ...
      if (allowance < fresh.rawAmount)
        throw fail(
          "The approval transaction confirmed, but the USDC allowance is not sufficient. Refresh before continuing to deposit.",
