...
export function withAccrual(position) {
  ...
  const endMs =
    position.status === "closed" ? Date.parse(position.updated_at) : Date.now();
...
function bestVaultFor(asset, excludeId) {
  const candidates = filter(
    "vaults",
    (v) => v.asset === asset && v.status === "active",
...
export function maybeRebalance(position) {
  ...
  const endMs =
    position.status === "active" ? Date.now() : Date.parse(position.updated_at);
  ...
  store.rebalances = store.rebalances
...
function weighted(vaults, field) {
  ...
  if (totalAlloc > 0) {
    return (
      vaults.reduce((s, v) => s + v[field] * (v.allocation_pct || 0), 0) /
      totalAlloc
    );
  }
...
export function yieldFor(asset = null) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    history.push({
      t: now - (29 - i) * dayMs,
      apy: round4(Math.max(0, blend_apy + drift)),
    });
  }
...
export function buildLedger(filters = {}) {
  ...
  for (const p of positions) {
    ...
    const events = filter(
      ...
      (e) =>
        e.position_id === p.id &&
        (e.type === "deposit" || e.type === "withdraw" || e.type === "close"),
  ...
  const typed = filters.type
    ? entries.filter((e) => e.type === filters.type)
    : entries;
...
function onchainValue(p, now) {
  ...
  const endMs =
    p.status === "closed" ? Date.parse(p.updated_at) : Math.min(dayStart, now);
...
export function reconciliationReport(scope = "all") {
  ...
  if (scope.startsWith("usr_"))
    positions = positions.filter((p) => p.user_id === scope);
  else if (scope.startsWith("pos_"))
    positions = positions.filter((p) => p.id === scope);
...
export function balanceSnapshots({ from, to, asset } = {}) {
  ...
  const positions = filter(
    "positions",
    (p) => p.principal > 0 && (!asset || p.asset === asset),
  );
  ...
  for (let day = Math.floor(fromMs / DAY) * DAY; day <= toMs; day += DAY) {
    ...
    snapshots.push({
      ...
      by_asset: assets.map((a) => ({
        ...a,
        accrued: round2(a.value - a.principal),
      })),
    });
  }
  return snapshots;
