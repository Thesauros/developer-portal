 ...
 * `*` satisfies any scope.
 *
 * No route under /api/v1/partners or /api/v1/partner/ is served by this app
 * today, so those two mappings cannot fire. They stay as a fail-safe default:
 * if such a route is ever added here it requires a partner scope instead of
 * silently falling through to plain read/write.
 */
export function requiredScope(method, pathname) {
  pathname = pathname.slice(pathname.indexOf("/api/v1/"));
  if (pathname.startsWith("/api/v1/keys")) return "keys:admin";
  if (pathname.startsWith("/api/v1/partners")) return "partner:admin";
...
export function paginate(request, items) {
  ...
  if (cursor) {
    try {
      const m = Buffer.from(cursor, "base64")
        .toString("utf8")
        .match(/^offset:(\d+)$/);
          ...
          if (!fr.allowed) {
            response = rFail.error(
              ...
              "Too many failed authentication attempts. Slow down and retry.",
              {
                ...
                headers: {
                  "Retry-After": String(
                    Math.max(1, fr.reset - Math.ceil(Date.now() / 1000)),
                  ),
                },
              },
            );
          } else {
            response = rFail.error("unauthorized", authed.error, {
              status: 401,
            });
          }
        } else {
