...
// Reserved documentation hosts (RFC 2606/6761) that can never receive a real
// delivery — dispatch simulates success for these in the sandbox.
const SIM_HOSTS = ['example.com', 'example.org', 'example.net'];
function isSimulatedTarget(url) {
  try {
    const host = new URL(url).hostname.toLowerCase();
    return (
      SIM_HOSTS.includes(host) ||
...
export async function dispatch(webhook, event, payload) {
  ...
  if (!guard.ok) {
    ...
    latency_ms = 0;
  } else if (isSimulatedTarget(webhook.url)) {
    // The sandbox is a deterministic simulation: reserved/example hosts can
    // never receive a real POST, so deliveries to them are simulated as
    // successful instead of logging pointless DNS failures.
    status = 'delivered';
    latency_ms = 42 + (evt.id.length % 40);
  } else {
    try {
