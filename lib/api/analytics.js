...
function vaultHistory(vaultId, apy) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    out.push({
      t: now - (29 - i) * DAY_MS,
      apy: round4(Math.max(0, apy + drift)),
    });
  }
  return out;
}
...
export function signals(asset = null) {
  ...
  rows.forEach((r, i) => {
    r.rank = i + 1;
    r.recommendation =
      i === 0 ? "overweight" : i >= rows.length - 1 ? "underweight" : "neutral";
  });
  return rows;
}
...
const REGIME_DESC = {
  rising:
    "Rates are trending up; favor locking higher yields and tilting to longer-horizon venues.",
  falling:
    "Rates are trending down; favor flexibility and capital preservation.",
  volatile:
    "Rate signals are unstable; the risk model discounts forecast confidence and widens penalties.",
  stable:
    "Rates are range-bound; optimize on risk-adjusted carry and low turnover.",
};
...
export function regime(asset = null) {
  ...
  const overall =
    order.find((r) => perAsset.some((p) => p.regime === r)) || "stable";
...
export function upliftReport({ user_id, asset } = {}) {
  ...
  const aaveApy = {};
  for (const v of filter("vaults", (v) => v.provider === "aave"))
    aaveApy[v.asset] = v.apy;
  ...
  return {
    ...
    totals: {
      ...
      uplift_vs_aave_pct:
        aave > 0 ? round4(((current - aave) / aave) * 100) : 0,
    },
    positions: rows,
  };
}
...
export function decisions({ user_id, position_id, asset } = {}) {
  ...
  for (const p of positions) {
    ...
    const alternatives = filter(
      "vaults",
      (v) => v.asset === p.asset && v.status === "active",
    )
      .map((v) => ({
        vault_id: v.id,
        name: v.name,
...
export function advisor() {
  ...
  const rebalanceCount = decisions({}).filter(
    (d) => d.type === "rebalance",
  ).length;
  bullets.push(
