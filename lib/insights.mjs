  ...
  return {
    ...
    vaults: VAULTS.map((vault, i) => {
      ...
      return {
        ...
        // Onchain history for the vaults this app reads directly.
        rebalances: (() => {
          const history = rebalanceHistory(vault.id);
          if (!history) return [];
          return history.events.slice(0, 60).map((e) => ({
            ...e,
            fromName: name(e.from),
            toName: name(e.to),
          }));
        })(),
        rebalanceHistoryComplete: rebalanceHistory(vault.id)?.complete ?? false,
...
// Daily vault rate (APR %) for the last `days` days, keyed by UTC day.
async function dailyRates(vault, days) {
  const raw = await getJson(
    `lending/${vault.token}/apr-ticks/${days <= 30 ? "30d" : "180d"}?network=${vault.network}`,
    600000,
  );
  return new Map(ticks(raw).map((t) => [Math.floor(t.t / 86400000), t.v]));
}

// Balances, deposits, withdrawals and earnings from the vault contracts.
// Earned = current value - deposited + withdrawn (exact). Daily earnings are
// estimated from the principal held that day and the vault's daily rate.
export async function accountInsights(owner) {
  ...
  const rows = await Promise.all(
    VAULTS.map(async (vault) => {
      const history = vaultEvents(vault.id);
      const mine = (history?.events || []).filter(
        (e) =>
          e.owner === address &&
          (e.kind === "deposit" || e.kind === "withdraw"),
      );
      const transactions = mine.map((e) => ({
        vaultId: vault.id,
        network: vault.network,
        token: vault.token,
        kind: e.kind,
        txHash: e.txHash,
        url: vault.explorer + "/tx/" + e.txHash,
        at: e.at,
        amount: e.amount,
      }));
      const deposited = mine
        .filter((e) => e.kind === "deposit")
        .reduce((s, e) => s + e.amount, 0);
      const withdrawn = mine
        .filter((e) => e.kind === "withdraw")
        .reduce((s, e) => s + e.amount, 0);
      const balance = await positionValue(vault.id, address).catch(() => null);
      const complete = !!history?.complete;
      const earned =
        complete && balance != null ? balance - deposited + withdrawn : null;
      // Principal held at the end of each of the last 30 days.
      let dailyEarned = [];
      if (complete && (mine.length || balance > 0)) {
        const rates = await dailyRates(vault, 30);
        const flows = [...mine].sort((a, b) => a.at - b.at);
        for (let d = today - 29; d <= today; d++) {
          const end = (d + 1) * 86400000;
          const principal = flows
            .filter((e) => e.at < end)
            .reduce(
      ...
      return {
        ...
        deposited,
        withdrawn,
        firstDepositAt: mine.length ? Math.min(...mine.map((e) => e.at)) : null,
        transactions,
        complete,
      };
    }),
