...
if (process.env.TURSO_DATABASE_URL) {
  ...
  if (process.env.VERCEL) {
    throw new Error(
      "Vercel requires persistent account storage: set TURSO_DATABASE_URL and TURSO_AUTH_TOKEN, then run scripts/init-auth.mjs before deploying.",
    );
  }
  const databasePath =
...
export async function institutionSession(headers) {
  ...
  return {
    ...session,
    user: { ...session.user, walletAddress: rows[0]?.address || null },
  };
}
...
if (!process.env.TURSO_DATABASE_URL) database.exec(treasurySchema);
